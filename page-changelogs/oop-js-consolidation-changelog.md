...
| 2026-08-01 | `partials/ui-nav-bar.blade.php` | Logout tooltip | Added `data-tip="Logout now ?"` to the top-bar logout button and nav-drawer logout. The CSS `[data-tip]::after` tooltip was invisible because `.top-bar` has `overflow: hidden` (tooltip renders above the button, off-screen/clipped). Fixed with a JS tooltip (`mouseover`/`mouseout` delegation listening for `[data-tip]`, appended to `<body>` with `position: fixed`) that escapes the clipped container and shows the tip below the button. Also fix `e.target.nodeType === 3` text-node guard for `.closest()`. Verified via Playwright: `#gTooltip` appears with text "Logout now ?" on hover, 0 console errors |
