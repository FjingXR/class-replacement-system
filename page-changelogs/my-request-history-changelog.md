...
| 2026-08-15 | Table rows | Row hover tooltips | Added `data-tip` to all `<tr>` rows — pipe-delimited summary (code, name, type, original date/time, replacement proposal, students, status). Uses shared `HtmlBuilder.rowTip()` from ui-common.js. Tooltip appears above the row. |
