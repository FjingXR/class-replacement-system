...
| 2026-08-15 | Table rows | Row hover tooltips | Added `data-tip` to all `<tr>` rows — pipe-delimited summary (code, name, type, date/time, venue, students, cohorts, conflict reason). Uses shared `HtmlBuilder.rowTip()` from ui-common.js. Tooltip appears above the row via `tr[data-tip]` CSS override. |
